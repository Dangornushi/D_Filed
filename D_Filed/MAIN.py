...
class Main:
	def __init__(self, _typ, mod, enc):
		...
		self.v1 = StringVar()
		self.txt = Text(self.frame1, fg="green", insertbackground="red", height=15, width=90)
	...
	def jikcp(self, event=None):
		f = open("cpp.bat", "w", encoding="utf_8")
		f.write("@echo off\ng++ -o a " + self.typ + "\na\npause\nexit")
		f.close
		ji = subprocess.run(["com"],shell=True)
	...
	def run(self):
		...
		self.l_f = Font(font="Cambrea", size=15)
		self.f = Font(family="Cambrea", size=20)
		...
		self.jik_menu.add_command(label="このpython言語のコードの実行(F5)", font = self.l_f,  command=self.jikp)
		self.jik_menu.add_command(label="このC言語のコードの実行(F6)", font = self.l_f,  command=self.jik)
		self.jik_menu.add_command(label="このC++言語のコードの実行(F7)", font = self.l_f,  command=self.jikcp)
		...
		self.txt.bind("<Control-F6>", self.jik)
		self.txt.bind("<Control-F7>", self.jikcp)
